...
struct RealtimePreviewState
{
  ...
  QVector3D boundsMin;
  QVector3D boundsMax;
  QVariant previousMinimumDistance;
};
...
QgsRectangle padded3DViewExtent( const QgsRectangle &extent )
{
  ...
  const double pad = std::max( std::max( padded.width(), padded.height() ) * 0.25, 0.1 );
  padded.grow( pad );
  return padded;
}
                                                     ...
                                                     QString *errorMessage )
{
  ...
  state.boundsMin = QVector3D( xmin, ymin, zmin );
  state.boundsMax = QVector3D( xmax, ymax, zmax );
  if ( newPreviewRoot )
    state.previousMinimumDistance = scene->cameraController()->property( "localMinimumZoomDistance" );
  scene->cameraController()->setProperty( "localMinimumZoomDistance",
    ParamModelerCamera::minimumDistance( ( state.boundsMax - state.boundsMin ).length() ) );
  scene->setProperty( "localPreviewBounds", QVariantList{ xmin, ymin, zmin, xmax, ymax, zmax } );
  if ( newPreviewRoot )
  {
  ...
  // disappear during zoom / rotation.
  {
    ...
    if ( mapSettings )
    {
      ...
      if ( newPreviewRoot )
      {
        ...
        // The QGIS close-zoom patch includes localPreviewBounds in its normal
        // near/far calculation, together with terrain and point-cloud layers.
      }
  ...
  if ( newPreviewRoot )
    focusRealtimePreviewMesh( iface );
  else
    scene->cameraController()->setCameraPose( scene->cameraController()->cameraPose(), true );
...
void ParamModelerScene3D::clearRealtimePreviewMesh( QgisInterface *iface )
{
  ...
  for ( Qgs3DMapCanvas *canvas : canvases )
  {
    ...
    {
      delete state.root;
      scene->setProperty( "localPreviewBounds", QVariant() );
      scene->cameraController()->setProperty( "localMinimumZoomDistance", state.previousMinimumDistance );
      scene->cameraController()->setCameraPose( scene->cameraController()->cameraPose(), true );
    }
  }
...
bool ParamModelerScene3D::focusRealtimePreviewMesh( QgisInterface *iface )
{
  if ( !iface ) return false;
  for ( Qgs3DMapCanvas *canvas : iface->mapCanvases3D() )
  {
    if ( !canvas || !canvas->scene() ) continue;
    const auto it = sRealtimePreviewMeshes.constFind( canvas->scene() );
    if ( it == sRealtimePreviewMeshes.constEnd() || !it->root || !it->mapSettings ) continue;
    const QVector3D center = ( it->boundsMin + it->boundsMax ) * 0.5f;
    const QgsVector3D origin = it->mapSettings->origin();
    QgsCameraController *controller = canvas->scene()->cameraController();
    Qt3DRender::QCamera *camera = controller->camera();
    const double distance = ParamModelerCamera::framingDistance(
      ( it->boundsMax - it->boundsMin ).length(), camera->fieldOfView(), camera->aspectRatio() );
    const QgsCameraPose pose = controller->cameraPose();
    controller->setLookingAtPoint( QgsVector3D( center.x() - origin.x(), center.y() - origin.y(), center.z() - origin.z() ),
                                   distance, pose.pitchAngle(), pose.headingAngle() );
    return true;
  }
  return false;
}
