...
- 明确点云分面里的 `top` 表示屋顶语义，不是只表示水平面：类似人字形屋顶，墙高以上的斜屋面也应归入屋顶显示/屋顶区域判断。棱台几何校正继续使用屋顶上部区域估计 `topLength/topWidth`，斜坡参与该屋顶区域，不单独剔除。
...
- 用户已反馈中文界面可用。本轮同步源码、翻译源文件及编译后的插件语言包、检查脚本和文档；未将本机 QGIS 构建目录语言包纳入插件仓库。
- 功能后续建议（尚未实现）：优先保存/恢复微调会话，其次撤销/重做，再评估点云到模型表面距离可视化。现有参数 JSON 与评估 CSV 不能代替完整会话恢复。

- 同日接入简体中文翻译：新增 `i18n/parammodeler_zh_CN.ts/.qm`，245 条完整翻译，通过插件资源嵌入。`initGui()` 按 QGIS 界面语言安装翻译器，卸载时移除；中文菜单、参数标签、对话框及操作提示不影响 CSV 字段、参数键和模型路径。
- 基元下拉框的 14 个内部类型名显式标记 `notr="true"`，避免翻译破坏 `currentText()` 驱动的基元选择。高度控制与 3D 拾取使用显式翻译上下文，修复无 Q_OBJECT 类的翻译查找问题。
- QGIS 本机语言包：从原有 `i18n/qgis_zh-Hans.ts` 生成 `build/output/i18n/qgis_zh_CN.qm`（28923 条译文）；这是经授权补充构建目录资源，不是修改 QGIS 核心源码。未更改用户语言设置；在选项中选择简体中文并重启生效。
- 验证：插件相关 4 个 C++ 文件 MSVC `/Zs` 通过；5 项 Qt 离屏翻译测试通过（含全部译文与占位符、资源加载、中文界面、英文回退、内部类型名）。QGIS 语言包可由 QTranslator 成功加载。开发侧未执行完整 DLL 链接；随后用户反馈中文界面可用，不将此反馈扩大为全部功能验收。

- 同日整理顶部菜单为 Scene / Export / Dataset：CSV、JSON 放在导出一级；网格 OBJ/STL 与模型采样 PLY/TXT 分组；输入点云转换与批量生成放入 Dataset，当前基元生成排在全部生成之前。
- 清除入口改名为“Clear plugin models and point clouds”，保留现有清理范围；打开菜单时根据输入文件和插件场景内容更新启用状态。CSV action 纳入 `.ui`，语法检查会重新生成临时 UI 头文件。

- 导出菜单新增 `Current evaluation CSV...`，UTF-8 BOM 编码，标准 CSV 引号转义，原子写入；每次保存独立快照。
- 两个回归入口均保存原始预测的 UI 语义映射、校正后值、当时的校正开关和实际模型/权重路径。v4-1 只训练了 HalfCylinderRoof、TruncatedPyramidRoof、LHouse、IndentedCuboid 四类，导出不统一写死版本。
- 导出当前物理参数、TX/TY/TZ 和旋转角；参数变化分别记录 corrected-raw、current-corrected，后者也可能包含 UI 限幅/舍入，不能全归因于人工修改。
- 对齐指标使用当前模型完整位姿变换后的 bbox，与匹配的已显示点云全点云稳健 bbox 比较。单独缓存全点云 bbox，避免把底部截面范围误当整个形体范围。差值统一为 model-cloud；z 最小/最大值差分别表示底部/顶部差。
- 无真值评估（ground_truth=not_evaluated）；bbox 差不是参数 MAE 或表面拟合误差。未推理、切换输入/基元后预测留空；没有对应显示点云时点云指标留空。仅修改插件。
- 验证：使用现有 VS 工程 include/define 执行 MSVC `/Zs` 语法检查；新增 `scripts/check_plugin_syntax.ps1` 可复查。尚未完整链接插件 DLL 或在 QGIS 中实测导出。
...
### 未发布工作区 (2026-09-16) — 第一版点云 3D 拾取对齐

在 v2.3.12 基础上实现 `Pick 3D target for model anchor`，尚未发布新版本。

- 从实际显示的 PointZ 图层读取 XYZ。使用当前相机 view/projection 矩阵和当前场景 origin 投影到屏幕，不重新读取归一化输入，不使用 aux。
- 鼠标附近 12 个逻辑像素内选点；同一像素距离档优先相机前方较近点，拒绝背后、视锥外、非有限坐标和空白点击。
- 激活时显示上部参考点红 X，悬停显示黄色候选框。按后续反馈，上部参考点改取原锚点正上方的实际网格顶点：长方体/L 型/凹槽体的外侧顶角、屋顶类檐口、圆柱顶面中心、圆顶/锥顶/塔顶点；再施加完整旋转和平移。TriPrismPyramid 不调整。原局部 `(0,0,0)` 生长锚点与旋转轴保持不变。
- 按本次要求，命中后同时更新 **TX/TY/TZ**，将源点移到真实目标 XYZ；这与上一节计划中只改 TX/TY 的范围不同。旋转和尺寸不变。
- 拾取时暂停相机导航；成功、右键、Esc、再次点按钮后退出并恢复原工具。切换模型、重载点云、清空场景、卸载插件时清理拾取状态；点云指针改为 QPointer，支持外部删除图层。
- `[ManualAlign3D]` 输出 source / target / translation 到 VS 和 QGIS 日志消息面板的 `ParamModeler` 页。
- 保留原有 bbox + 底部截面自动对齐和一步式 2D 点选平移。

**第一版范围与限制：** 当前插件创建的 PointZ 点云、局部 3D 场景、与点云相同的 CRS；使用实时模型所在的第一个已初始化 3D 画布。拾取的是采样点，不拟合缺失角点或圆柱中心；屏幕深度择近不是完整渲染深度遮挡测试，模型或其他图层遮挡点云时可开启线框模式或换视角后再拾取。建议先调整视角再点按钮。

**验证：** MSVC C++17 语法检查通过（`parammodeler_dock.cpp` / `parammodeler_pick3d.cpp`）；`tests/test_pick3d_math.cpp` 的 15 项测试通过，覆盖透视/正交、旋转、浮动原点、视锥裁剪、屏幕 Y、视口缩放、空白拒绝及重叠点深度选择。插件增量构建请求未获批准，未生成新 DLL，未做 QGIS 实机交互验证。

#### 后续反馈：红 X 不可见 + 底部缺点云

- 原红 X 由世界空间短线通过 QgsRubberBand3D 绘制。用户反馈不可见；当前尚未实机复现具体原因，改用屏幕空间三角形，在 QGIS 的置顶 rubber-band pass 中绘制白描边红 X 和黄框，取消对线几何着色器、世界空间线长和地形高度缩放的依赖。
- 红 X 固定屏幕大小；按实际 3D 坐标投影，参数、场景原点、窗口尺寸变化后刷新。标记不写深度，避免改变场景深度数据。
- 上角通过网格中位于本地锚点竖线上的最高实际顶点确定，不使用整体 bbox 最高 Z；例如人字屋顶选檐口，不把参考点悬在墙角上方的屋脊高度。
- 本轮 MSVC 语法检查通过；22 项投影/选点/上角测试通过；OpenGL 离屏测试在 400x300、800x600、1600x1200 下均检测到红 X、白色描边和黄色候选框，已查看输出图像。未执行完整插件构建或 QGIS 实机回归，需用户编译后试用。

#### 主面板整理

- 基元类型区只保留类型选择；估计入口、3D/2D 对齐与线框开关移到独立的点云与对齐区。
- 对齐按钮缩短为 `Align in 3D` / `Align in 2D`，3D 在前；恢复预测改名 `Reset prediction`，与随机参数一同放在模型参数区。
- 形状参数放在位姿参数之前；参数页按当前基元内容确定高度，避免简单模型继承复杂模型的大块空白。
- 几何校正只在估计对话框显示，状态改存为成员布尔值，默认关闭、会话内保留，原校正算法与启用条件不变。
- MSVC 语法检查通过；用原始 .ui 加对应动态布局制作的 Qt 离屏预览，在 360/640 宽度检查按钮完整显示和页面切换高度，并查看截图。该预览不替代 QGIS 实机验证；未执行完整插件构建。

#### 高度调整方向 (2026-09-17)

- 13 类的 16 个实际高度/深度参数行增加 24px 上下箭头；TriPrismPyramid 和无量纲比例不加开关。每行独立记住本次会话的模式，切换本身不修改参数。
- 普通高度默认 ↑（固定下端）；↓ 时用 `R * (0,0,oldHeight-newHeight)` 补偿整栋模型的平移。总高度锁最高面/屋脊，HalfCylinderRoof 墙高锁檐口及屋顶，塔楼分段锁该段上端及以上结构。高度引起的内部比例缩放仍保持原模型定义，锁屋脊不等于整个斜屋顶形状不变。
- IndentedCuboid 的 innerHeight 是从顶面向下的凹槽深度：默认 ↓ 固定上沿，与原行为一致；↑ 时反向补偿整栋平移以固定凹槽底面。这不是只移动局部凹槽的独立编辑。
- 人工补偿独立于 3D 拾取参考点和模型原点。推理回填/恢复预测与随机生成使用作用域保护；批量数据生成由已有更新标志保护。被跳过的更新仍刷新高度基线，避免下一次手动编辑跳变。
- 平移超出允许范围时同时撤回高度修改。旋转模型沿本地高度方向补偿 XYZ，精度受现有位姿输入框小数位限制。
- 修正滑块反向联动：spin 更新 slider 时阻断回传，并改用四舍五入，防止 0.01 高度补偿被 0.1 位姿滑块吞掉；形状输入框直接连接预览调度，保留键盘和精调刷新。
- MSVC 语法检查通过（dock / pointnet / randomizer）；`tests/test_heightcontrol.cpp` 的 11 项交互与几何测试通过，覆盖增高、缩短、滑块、步进、模式切换、批量回填基线、边界撤回、凹槽深度和倾斜补偿。未执行插件 DLL 完整构建或 QGIS 实机验证。

#### QGIS 核心实验的备份与回退 (2026-09-17)

- **原则：只改自己的插件。** 本次近距离缩放方案修改了宿主 QGIS 核心，属于实验性额外依赖，不能作为标准 QGIS 安装上直接可用的插件功能。以后优先插件内方案；必须改核心时，先单独说明影响、取得确认并提前备份。
- **实验内容**：用模型尺寸决定缩放下限，将实时模型包围盒纳入动态裁剪，取消插件固定 `near=0.001, far=1e9` 回调，增加聚焦模型按钮。不改变模型/点云坐标或对齐算法。部署需要同时编译并使用 `qgis_3d` 和 `plugin_parammodeler`。上一轮完成语法与相机相关测试，但未完整构建 DLL 或做 QGIS 实机验证。
- **核心范围**：QGIS 根目录下 `src/3d/qgscamerapose.h`、`qgscamerapose.cpp`、`qgscameracontroller.cpp`、`qgs3dmapscene.cpp`。这些文件不在插件 Git 仓库内，不能只靠插件 Git 回退。
- **配套插件范围**：`parammodeler_scene3d.cpp/.h`、`parammodeler_dock.cpp`。回退必须同时还原核心与这些配套修改，否则旧核心和新插件裁剪逻辑不匹配。恢复后返回原缩放限制和旧裁剪方案，移除本轮聚焦按钮，但保留之前的高度箭头、上部红 X、3D 拾取及界面整理。
- **备份位置**：插件目录 `backups/2026-09-17-before-qgis-close-zoom/`。`before/` 保存 7 个文件的修改前源码，`after/` 保存实验版快照，`manifest.json` 保存来源和 SHA-256 校验值。核心修改前源码由已记录补丁反向重建，并正向重放核验；这是事后重建备份，不是事前快照，也不声称保留未知的原始换行格式。scene3d 两文件使用已检查为仅含本轮修改的 HEAD 基线；dock 只反向移除本轮改动，保留之前未提交的功能。
- **校验/回退**：在插件目录执行 `python patches/restore_close_zoom_backup.py` 只校验，不改源码。用户决定回退后，关闭 QGIS，在具备核心目录写权限的环境执行同一命令加 `--apply`，再重新编译并部署 `qgis_3d` 和 `plugin_parammodeler`。没有修改前 DLL 备份，源码恢复不等于运行中的二进制已回退。
